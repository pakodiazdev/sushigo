...
- **Full CI/CD pipeline** — one orchestrated GitHub Actions workflow (`ci.yml`) gates every PR through a single dependency graph (backend/frontend lint + tests, plus the full Cypress E2E suite run headless against the real Docker stack) behind one stable required check, `ci-gate` (see [TD-06](doc/decisions/td-06-unified-ci-dag.md)), plus a one-click manual Cloud Run preview deploy via GCP Workload Identity Federation.
