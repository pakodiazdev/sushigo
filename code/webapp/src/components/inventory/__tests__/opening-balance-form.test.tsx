...
function resetHookState() {
  ...
  hookState.value.canSubmit = true
...
describe('OpeningBalanceForm (view)', () => {
  ...
  it('renders Spanish reference-field copy', () => {
    ...
    expect(getByText('Referencia')).toBeDefined()
    expect(getByText(/Folio o identificador que podrás buscar/i)).toBeDefined()
  })
  ...
  it('disables posting until the current preview succeeds', () => {
    hookState.value.canSubmit = false
    const { container } = render(<OpeningBalanceForm {...defaultProps} />)
    expect(container.querySelector('button[type="submit"]')?.hasAttribute('disabled')).toBe(true)
  })
})
