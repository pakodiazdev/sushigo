...
const openingBalanceSchema = z.object({
  ...
  reference: z.string().max(255, 'La referencia no puede exceder 255 caracteres'),
  notes: z.string(),
})
...
export function useOpeningBalanceForm({
  ...
  // Posting is permanent. Require a successful preview for the exact current
  // numeric inputs, rather than merely hiding a stale/failed preview while the
  ...
  const allErrors = {
    ...
    reference: errors.reference?.message || validationErrors.reference,
