...
interface ScopeFormStepProps {
  readonly register: UseFormRegister<OverrideScopeFormValues>
  readonly errors: FieldErrors<OverrideScopeFormValues>
  ...
  return (
    <>
      ...
      <div className="flex justify-end gap-2 border-t px-5 py-3">
        ...
        <Button type="button" size="sm" disabled={!isValid || isPending} onClick={onPrimaryClick}>
...
// ── Step 2: conflicts confirmation ───────────────────────────────────────────

interface ScopeConflictsStepProps {
  readonly dayLabel: string
  readonly conflicts: ScheduleDayOverride[]
  readonly isError: boolean
  readonly isPending: boolean
...
function ScopeConflictsStep({ dayLabel, conflicts, isError, isPending, onBack, onConfirm }: ScopeConflictsStepProps) {
  return (
    <>
      ...
      <div className="flex justify-end gap-2 border-t px-5 py-3">
        <Button type="button" variant="outline" size="sm" onClick={onBack}>← Volver</Button>
...
export function OverrideScopeDialog({ isOpen, dayLabel, dayOfWeek, existingOverrides, isPending, isError, onSubmit, onClose }: OverrideScopeDialogProps) {
  // The parent clears `dayLabel`/`dayOfWeek`/`existingOverrides` the instant it
  // closes this dialog, so cache the last non-null content for the exit animation.
  const lastContent = useRef({ dayLabel, dayOfWeek, existingOverrides })
  if (isOpen) lastContent.current = { dayLabel, dayOfWeek, existingOverrides }
  const { dayLabel: cDayLabel, dayOfWeek: cDayOfWeek, existingOverrides: cExistingOverrides } = lastContent.current
  ...
  const {
    ...
    backToForm,
  } = useOverrideScopeDialog(isOpen, cDayOfWeek, cExistingOverrides, onSubmit)
  ...
  return createPortal(
    <div className="fixed inset-0 z-[300] flex items-center justify-center px-4">
      <button type="button" aria-label="Cerrar" className={`absolute inset-0 w-full border-0 bg-black/50 p-0 ${backdropCls}`} onClick={onClose} />
      <div className={`relative z-10 w-full max-w-sm rounded-lg border border-border bg-background shadow-xl ${panelCls}`}>
        <div className="flex items-center justify-between border-b px-5 py-4">
          <div className="flex items-center gap-2">
            ...
            <h3 className="text-base font-semibold">Ajuste — {cDayLabel}</h3>
          </div>
