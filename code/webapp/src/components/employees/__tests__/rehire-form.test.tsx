...
vi.mock('@/components/employees/use-rehire-form', () => {
...
describe('RehireForm without branch', () => {
  afterEach(() => { cleanup() })
  ...
  it('shows warning text when no branch available', async () => {
    vi.doMock('@/components/employees/use-rehire-form', () => {
      const { useForm } = require('react-hook-form')
      return {
        useRehireForm: () => {
          const form = useForm({ defaultValues: { start_date: '2026-03-31' } })
          return { form, today: '2026-03-31', effectiveBranch: null }
        },
      }
    })

    const { RehireForm: ReForm } = await import('@/components/employees/rehire-form')
    const { getByText } = render(
      <ReForm isLoading={false} onSubmit={vi.fn()} onCancel={vi.fn()} />
    )
    expect(getByText(/No hay sucursal disponible/)).toBeDefined()
  })
})
