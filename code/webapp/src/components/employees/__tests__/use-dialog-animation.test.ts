...
import { useDialogAnimation, animCls } from '@/components/employees/use-dialog-animation'
...
// ── animCls helper function ──────────────────────────────────────────────────

describe('animCls', () => {
    it('returns enterVal when animating is "enter"', () => {
        expect(animCls('enter', 'enter-class', 'exit-class')).toBe('enter-class')
    })

    it('returns exitVal when animating is "exit"', () => {
        expect(animCls('exit', 'enter-class', 'exit-class')).toBe('exit-class')
    })

    it('returns empty string when animating is null', () => {
        expect(animCls(null, 'enter-class', 'exit-class')).toBe('')
    })
})
