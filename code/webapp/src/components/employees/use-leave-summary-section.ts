...
export function useLeaveSummarySection(employeeId: string, employee: { id: string; first_name: string; last_name: string; code: string } | undefined) {
  ...
  // Summary query: current month, all results
  const summaryQuery = useEmployeeLeaves(employeeId, { date_from: monthStart, date_to: monthEnd, per_page: 100 })
