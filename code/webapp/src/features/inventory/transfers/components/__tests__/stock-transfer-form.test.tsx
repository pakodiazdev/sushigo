  ...
  lines: [
    {
      ...
      base_uom: { id: 'u-kg', code: 'KG', symbol: 'kg' },
      entry_quantity: 5,
      conversion_factor: 1,
      base_quantity: 5,
...
describe('StockTransferForm', () => {
  ...
  it('surfaces a server-side reference error on the reference field', () => {
    formMutation.validationErrors = { reference: 'La referencia no puede exceder 255 caracteres.' }

    const view = render(<StockTransferForm transfer={draftTransfer} onSuccess={vi.fn()} onCancel={vi.fn()} />)

    expect(view.getByText('La referencia no puede exceder 255 caracteres.')).toBeDefined()
  })

  it('rejects a reference longer than the API limit before submitting', async () => {
    const view = render(<StockTransferForm transfer={draftTransfer} onSuccess={vi.fn()} onCancel={vi.fn()} />)

    fireEvent.change(view.getByLabelText('Referencia'), { target: { value: 'x'.repeat(256) } })
    fireEvent.click(view.getByRole('button', { name: /actualizar traslado/i }))
