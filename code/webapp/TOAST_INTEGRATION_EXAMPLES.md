...
export function OpeningBalanceForm({ onSuccess, onCancel }: OpeningBalanceFormProps) {
...
export function StockOutForm({ onSuccess, onCancel }: StockOutFormProps) {
  ...
  useEffect(() => {
    if (currentStock && currentStock.available < selectedVariant?.min_stock) {
      showWarning(
...
export default function ItemVariantsPage() {
  ...
  const handleFormSuccess = () => {
    ...
    setShowForm(false)
    showSuccess(
      editingVariant ? 'Variant updated successfully' : 'New variant created',
      'Success'
    )
  }
