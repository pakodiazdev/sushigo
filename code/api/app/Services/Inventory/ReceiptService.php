...
class ReceiptService
{
    ...
    private function reverseReceiptLine(Receipt $receipt, ReceiptLine $line, int $userId, ?string $reason): void
    {
        ...
        $originalMovementLine = $originalMovement->lines->first()
        ...
        $originalUnitCost = (float) $originalMovementLine->unit_cost;

        // #579: the movement's own line_total is the receipt line's
        // authoritative exact value (net_acquisition_amount) — reversal must
        // subtract exactly that, not qty * unit_cost reconstructed from the
        // already-rounded rate, which does not recover it (see
        // Stock::applyWeightedAverageCost()'s $lineValue parameter, used at
        // posting time for this same reason).
        $originalLineValue = $originalMovementLine->line_total !== null
            ? (float) $originalMovementLine->line_total
            : null;
        ...
        try {
            ...
            // immutable evidence the original posting recorded — never a
            // recomputed approximation. Refuses (409) rather than silently
            // dropping value when intervening operations make the removal
            // unreconcilable — see WeightedAverageCostCalculator::subtractValue()/subtractExactValue().
            $stock->reverseWeightedAverageCost($baseUnits, $originalUnitCost, $originalLineValue);
