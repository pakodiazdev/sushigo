...
class StockTransferService
{
    ...
    private function lockEndpoints(StockTransfer $transfer, bool $requireActive = true): array
    {
        ...
        $unitIds = $locked->pluck('operating_unit_id')->map(fn ($id) => (int) $id)->unique()->sort()->values();
        $liveUnitCount = OperatingUnit::query()
            ->whereIn('id', $unitIds)
            ->orderBy('id')
            ->lockForUpdate()
            ->get()
        ...
        if ($liveUnitCount !== $unitIds->count()) {
            throw new StockTransferLocationUnavailableException(
                "Stock Transfer #{$transfer->id} references a location whose Operating Unit is no longer available."
            );
        }
    ...
    private function lockAndAssertVariantsAvailable(StockTransfer $transfer, $lines): void
    {
        ...
        foreach ($variantIds as $variantId) {
            ...
            if ($lines->where('item_variant_id', $variantId)->contains(
                fn (StockTransferLine $line) => (int) $line->base_uom_id !== (int) $variant->uom_id
            )) {
                throw new StockTransferVariantUnavailableException(
                    "Stock Transfer #{$transfer->id} references a Product Variant whose base UOM changed after drafting."
                );
            }
        }
    }
