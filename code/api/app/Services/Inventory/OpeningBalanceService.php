...
class OpeningBalanceService
{
    ...
    private function calculateValuation(?float $unitCost, float $conversionFactor, float $baseQuantity): array
    {
        ...
        // Round only the values that get persisted, never an intermediate.
        // The converted base cost can carry more than four decimals; rounding
        // it before multiplying by base quantity scales that residual by the
        // whole balance, so a large opening balance would record a materially
        // wrong immutable total. Keep the full-precision cost for the product
        // and round each stored figure once, at the end.
        $baseCostExact = $conversionFactor != 0 ? $unitCost / $conversionFactor : 0.0;

        $baseCost = round($baseCostExact, self::MONEY_SCALE);
        $this->assertLedgerMoneyFits($baseCost, 'The converted unit cost exceeds the ledger\'s decimal(15,4) range.');

        $totalValue = round($baseQuantity * $baseCostExact, self::MONEY_SCALE);
    ...
    private function assertAccumulatedStockFits(int $locationId, int $variantId, float $baseQuantity): void
    {
        ...
        // Snap to the ledger's decimal(15,4) scale before the boundary check so
        // this preview agrees with Stock::increaseOnHand() at the ceiling —
        // otherwise a raw binary-float sum can 422 a balance the column stores.
        $accumulated = round($currentOnHand + $baseQuantity, 4);

        if (! is_finite($accumulated) || $accumulated > Stock::MAX_STORED_QUANTITY) {
            throw ValidationException::withMessages([
                'quantity' => 'The quantity would make the accumulated stock exceed decimal(15,4).',
            ]);
        }
