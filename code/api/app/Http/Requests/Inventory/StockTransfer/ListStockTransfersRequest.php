...
class ListStockTransfersRequest extends FormRequest
{
    ...
    {
        return $this->resolveLocationPublicId('source_location_id');
    }

    public function destinationLocationId(): ?int
    {
        return $this->resolveLocationPublicId('destination_location_id');
    }
    ...
    private function resolveLocationPublicId(string $field): ?int
    {
        $publicId = $this->validated($field);

        if (! $publicId) {
            return null;
        }

        return InventoryLocation::withTrashed()->where('public_id', $publicId)->value('id');
    }
}
