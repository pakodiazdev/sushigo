...
{
    protected function accessibleLocationRule(bool $activeOnly = false): Exists
    {
        $rule = Rule::exists('inventory_locations', 'public_id');

        if ($activeOnly) {
            $rule->withoutTrashed()->where(fn (Builder $query) => $query
                ->where('is_active', true)
                ->whereExists(fn (Builder $query) => $query
                    ->selectRaw('1')
                    ->from('operating_units')
                    ->whereColumn('operating_units.id', 'inventory_locations.operating_unit_id')
                    ->whereNull('operating_units.deleted_at')));
        }
