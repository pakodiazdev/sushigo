...
abstract class StockTransferRequest extends FormRequest
{
    ...
    protected function transferRules(): array
    {
        return [
            ...
            'destination_location_id' => ['required', 'string', 'different:source_location_id', $this->accessibleLocationRule(activeOnly: true)],
            'reference' => ['nullable', 'string', 'max:255'],
            'transfer_date' => ['required', 'date'],
            'notes' => ['nullable', 'string'],
            'lines' => ['required', 'array', 'min:1'],
            'lines.*.item_variant_id' => ['required', 'string', Rule::exists('item_variants', 'public_id')->withoutTrashed()->where('is_active', true)],
            'lines.*.entry_uom_id' => ['required', 'string', Rule::exists('units_of_measure', 'public_id')->where('is_active', true)],
