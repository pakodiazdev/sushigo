 ...
 *   @OA\Response(response=422, description="Validation Error. A concurrent code collision also returns rejected_code and suggested_code.", @OA\JsonContent(ref="#/components/schemas/ResponseError"))
 * )
 */
class CreatePurchasePresentationTemplateController extends Controller
{
    public function __invoke(
        StorePurchasePresentationTemplateRequest $request,
        PurchasePresentationTemplateCodeGenerator $generator,
    ): PurchasePresentationTemplateResource|JsonResponse {
        $data = $request->templateData();
        try {
            $template = DB::transaction(fn () => PurchasePresentationTemplate::create($data));
        } catch (UniqueConstraintViolationException) {
            $uom = UnitOfMeasure::findOrFail($data['compatible_dimension_uom_id']);

            return response()->json([
                'message' => StorePurchasePresentationTemplateRequest::DUPLICATE_CODE_MESSAGE,
                'errors' => ['code' => [StorePurchasePresentationTemplateRequest::DUPLICATE_CODE_MESSAGE]],
                'rejected_code' => $data['code'],
                'suggested_code' => $generator->suggest($data['package_type'], $data['base_unit_quantity'], $uom),
            ], 422);
        }
