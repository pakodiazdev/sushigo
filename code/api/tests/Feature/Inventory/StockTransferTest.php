...
class StockTransferTest extends InventoryTestCase
{
    ...
    public function creating_a_draft_persists_lines_and_changes_no_stock(): void
    {
        ...
        $response = $this->postJson('/api/v1/inventory/transfers', $this->draftPayload())
            ...
            ->assertJsonPath('data.lines.0.base_uom.id', $this->uomKg->public_id)
            ->assertJsonPath('data.lines.0.base_uom.symbol', 'kg');
    ...
    #[Test]
    public function a_draft_cannot_be_created_or_updated_with_inactive_endpoints(): void
    {
        ...
        $this->destination->update(['is_active' => true]);
        $id = $this->createDraft();
        $this->location->update(['is_active' => false]);

        $this->putJson("/api/v1/inventory/transfers/{$id}", $this->draftPayload())
            ->assertStatus(422)
            ->assertJsonValidationErrors('source_location_id');
    }

    #[Test]
    public function a_draft_cannot_use_a_location_whose_operating_unit_was_deleted(): void
    {
        $this->operatingUnit->delete();

        $this->postJson('/api/v1/inventory/transfers', $this->draftPayload())
            ->assertStatus(422)
            ->assertJsonValidationErrors(['source_location_id', 'destination_location_id']);
    }

    #[Test]
    public function a_draft_cannot_be_created_or_updated_with_an_inactive_variant(): void
    {
        $this->variant->update(['is_active' => false]);

        $this->postJson('/api/v1/inventory/transfers', $this->draftPayload())
            ->assertStatus(422)
            ->assertJsonValidationErrors('lines.0.item_variant_id');

        $this->variant->update(['is_active' => true]);
        $id = $this->createDraft();
        $this->variant->update(['is_active' => false]);
    ...
    #[Test]
    public function a_draft_line_rejects_an_inactive_entry_uom(): void
    {
        $this->uomKg->update(['is_active' => false]);

        $this->postJson('/api/v1/inventory/transfers', $this->draftPayload())
            ->assertStatus(422)
            ->assertJsonValidationErrors('lines.0.entry_uom_id');
    }
    ...
    #[Test]
    public function inverse_conversion_validation_uses_the_persisted_factor_precision(): void
    {
        $largeUom = UnitOfMeasure::create([
            'code' => 'HUGE', 'name' => 'Huge unit', 'symbol' => 'huge', 'type' => 'WEIGHT',
            'precision' => 4, 'is_base' => false, 'is_active' => true,
        ]);
        UomConversion::create([
            'from_uom_id' => $this->uomKg->id,
            'to_uom_id' => $largeUom->id,
            'factor' => 671140.939597,
            'tolerance_percent' => 0.5,
            'is_active' => true,
        ]);

        $this->postJson('/api/v1/inventory/transfers', $this->draftPayload([
            ['item_variant_id' => $this->variant->public_id, 'entry_uom_id' => $largeUom->public_id, 'entry_quantity' => 40],
        ]))->assertStatus(422)->assertJsonValidationErrors('lines.0.entry_quantity');
    }

    #[Test]
    public function a_conversion_removed_after_validation_returns_a_controlled_conflict(): void
    {
        $this->mock(StockTransferService::class)
            ->shouldReceive('createDraft')
            ->once()
            ->andThrow(new UomConversionNotFoundException('La conversión dejó de estar disponible.'));

        $this->postJson('/api/v1/inventory/transfers', $this->draftPayload([
            ['item_variant_id' => $this->variant->public_id, 'entry_uom_id' => $this->uomGr->public_id, 'entry_quantity' => 1000],
        ]))->assertStatus(409)->assertJsonPath('status', 409);
    }

    #[Test]
    public function creating_a_draft_locks_the_selected_uom_conversion(): void
    {
        $queries = [];
        ...
        $conversionLock = collect($queries)->first(fn (string $sql) => str_contains($sql, 'from "uom_conversions"')
            && str_contains($sql, 'for update'));

        $this->assertNotNull($conversionLock, 'Expected draft creation to lock the selected UOM conversion.');
    }
    ...
    #[Test]
    public function posting_is_rejected_when_an_endpoint_operating_unit_is_deleted_after_the_draft(): void
    {
        $this->seedSourceStock();
        $id = $this->createDraft();
        ...
        $admin->givePermissionTo('stock.manage');
    ...
    #[Test]
    public function posting_is_rejected_when_the_variant_base_uom_differs_from_the_draft_snapshot(): void
    {
        $this->seedSourceStock();
        $id = $this->createDraft();

        ItemVariant::whereKey($this->variant->id)->update(['uom_id' => $this->uomGr->id]);
    ...
    #[Test]
    public function transfer_history_can_be_filtered_by_an_archived_location(): void
    {
        $this->createDraft();
        $destinationPublicId = $this->destination->public_id;
        $this->destination->delete();

        $this->getJson("/api/v1/inventory/transfers?destination_location_id={$destinationPublicId}")
            ->assertOk()
            ->assertJsonPath('meta.total', 1)
            ->assertJsonPath('data.0.destination_location.id', $destinationPublicId);
    }
    ...
    {
        $this->postJson('/api/v1/inventory/transfers', $this->draftPayload([
            // GR -> KG reduces the base quantity into range; validation must
            // still reject the exact 12-digit entry value before decimal(15,4)
            // rounds the float-backed max rule up to 100000000000.
            ['item_variant_id' => $this->variant->public_id, 'entry_uom_id' => $this->uomGr->public_id, 'entry_quantity' => 100000000000],
        ]))->assertStatus(422)->assertJsonValidationErrors('lines.0.entry_quantity');
    }
