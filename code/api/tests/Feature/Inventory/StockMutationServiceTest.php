...
class StockMutationServiceTest extends InventoryTestCase
{
    ...
    #[Test]
    public function it_rejects_a_first_receipt_quantity_that_would_overflow_the_stock_quantity_column(): void
    {
        $variant = $this->createItemVariant($this->createItem());

        try {
            DB::transaction(fn () => $this->service->receiveInto(
                $this->location->id,
                $variant->id,
                Stock::MAX_STORED_QUANTITY + 1,
            ));
            $this->fail('Expected the oversized first-receipt quantity to be rejected before it reaches PostgreSQL.');
