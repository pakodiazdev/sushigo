...
class OpeningBalanceTest extends InventoryTestCase
{
    ...
    #[Test]
    public function it_derives_the_immutable_total_from_the_full_precision_converted_cost()
    {
        // A conversion whose factor does not divide the unit cost evenly: the
        // converted base cost repeats (100 / 7 = 14.285714...). Rounding it to
        // decimal(15,4) *before* multiplying by a large base quantity would
        // shave a material amount off the immutable line total, so the total
        // must be computed from the unrounded cost and rounded once at the end.
        $pack = UnitOfMeasure::create([
            'code' => 'PACK7',
            'name' => 'Seven-kilo pack',
            'symbol' => 'pk',
            'type' => 'WEIGHT',
            'precision' => 3,
            'is_base' => false,
        ...
        $item = $this->createItem(['name' => 'Bulk Nori']);
        $variant = $this->createItemVariant($item, ['uom_id' => $this->uomKg->id]);

        $data = $this->postJson('/api/v1/inventory/opening-balance', [
            'inventory_location_id' => $this->location->public_id,
            'item_variant_id' => $variant->public_id,
            'quantity' => 100000,
            'uom_id' => $pack->public_id,
            'unit_cost' => 100,
        ])->assertStatus(201)->json('data');

        // 100000 PACK7 * 7 = 700000 KG at a base cost of 100 / 7 rounded to 4dp.
        $this->assertEquals(700000, (float) $data['base_quantity']);
        $this->assertEquals(14.2857, (float) $data['base_cost']);
