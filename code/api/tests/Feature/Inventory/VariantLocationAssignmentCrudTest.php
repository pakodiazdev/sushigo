...
class VariantLocationAssignmentCrudTest extends InventoryTestCase
{
    ...
    public function it_locks_the_assignment_before_checking_stock_during_unassignment(): void
    {
        ...
        $assignmentLock = array_key_first(array_filter($queries, fn ($sql) => str_contains($sql, 'variant_location_assignments') && str_contains($sql, 'for update')));
        $stockLock = array_key_first(array_filter($queries, fn ($sql) => str_contains($sql, 'from "stock"') && str_contains($sql, 'for update')));
