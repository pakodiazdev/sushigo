...
class StockTest extends TestCase
{
    ...
    #[Test]
    public function fully_depleting_understated_rounded_rate_still_zeroes_the_exact_accumulator(): void
    {
        // Code review finding (#579 PR #626): the prior full-depletion fix only
        // clamped the *over*-removal case (qty * weighted_avg_cost exceeding
        // total_value). Here the rounded rate *understates* the accumulator
        // instead — 30,000 units @ total_value 4.0000 stores a rounded average
        // of 0.0001 (true ratio is 4/30000 = 0.00013333...), so a full
        // depletion at that rounded rate computes 30,000 * 0.0001 = 3.0000,
        // 1.0000 less than the exact total_value. min()'s clamp never engages
        // here since 3.0000 is already the smaller value — the fix must
        // recognize a full depletion (resultingOnHand === 0) and remove the
        // whole accumulator regardless of what the rounded rate computes.
        $stock = $this->makeStock(0, 0);
        ...
        $stock->decreaseOnHand(30000);
        $stock->refresh();

        $this->assertEquals(0.0, (float) $stock->on_hand);
        $this->assertEquals(0.0, (float) $stock->total_value);

        // A residual would otherwise silently inflate the next receipt's
        // blended average — confirm it doesn't.
        $stock->increaseOnHand(1);
        $stock->applyWeightedAverageCost(1, 1.0);
        $stock->refresh();

        $this->assertEquals(1.0, (float) $stock->weighted_avg_cost);
    }
