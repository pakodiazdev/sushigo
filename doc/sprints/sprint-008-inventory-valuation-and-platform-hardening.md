...
| ✅ | #552 | Opportunistic (§5.4): removed the `#490` quarantine guard from `employee-avatar-upload.cy.ts` — the actual root cause (Chrome's `crypto.randomUUID()` secure-context failure on `e2e-ci`'s non-localhost origin) had already been fixed by #545's shared `cypress.config.ts` exemption, so no app/config change was needed here. Also fixed an independent submit-button busy-state race (same pattern as #545) and, after Codex review, a retry-safety issue (fixed employee email/name colliding across a `retries=2` CI rerun). | PR #627 | — | 18m | 3/3 green on a fresh dev-lab E2E stack; no regression in `item-media-gallery-uploader.cy.ts`/`employees.cy.ts`; ESLint/TypeScript clean; 1/1 Codex review thread resolved (retry-safety finding); rebased twice past concurrently-merged sibling quarantine-spec PRs (`#553`, `#554`, `#535`) touching the same sprint doc; PR ready, merge pending. |
