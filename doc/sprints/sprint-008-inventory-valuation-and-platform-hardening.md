...
| Completed | — |
| Active workdays | — |
| Progress (Issues completed) | **10 / 10 (100%)** as of 2026-09-14 — all scoped Issues merged to `main` and Done (see §13) |
...
| 2026-09-13 | #552 | Fix quarantined Cypress spec: employee-avatar-upload.cy.ts (CI-only) | #545's shared `cypress.config.ts` secure-context fix (its own commit message flagged #552/#556 as likely also unblocked) made this a near-zero-cost follow-up while closing out that sibling fix | PR #627 opened — quarantine guard removed, submit-button busy-state race fixed and made retry-safe (Codex finding addressed), 3/3 green on dev-lab E2E, linters clean, review thread resolved; merged to `main` (`1116db34`). |
| 2026-09-13 | #553 | Fix quarantined Cypress spec: schedule-indefinite-summary.cy.ts (CI-only) | Same class of conflict-free quarantined-spec restoration as #545/#546/#550 (this sprint's Round 0); picked up opportunistically via `/issue-no-review` alongside the sprint's other lanes, even though `#553` itself was outside this sprint's originally-scoped three (§5.2) | ✅ Fixed scroll overshoot behind the panel header by scrolling directly to schedule text; removed quarantine and the non-retriable navigation warm-up; 1.3h tracked; PR #629; merged to `main` (`720fead2`). |
| 2026-09-13 | #554 | Fix quarantined Cypress spec: cash-register-code-suggestion.cy.ts (CI-only) | Same class of conflict-free quarantined-spec restoration as #545/#546/#550 (this sprint's Round 0), picked up via `/issue-no-review` while Round 2/3 lanes are in flight | Root-caused to a missing admin-role permission grant plus a Devtools/UI overlap, both fixed; see §13 |
...
|---|---:|---|---:|---|---:|---|
| ✅ | #415 | Delivered as Phase 1 of a scope split (per the Issue's own "bounded phases" requirement): `Money`/`Decimal` PHP primitives, the full monetary-field inventory/ADR, and the Purchase Receipt DTO/calculation float-boundary fix. Every other named domain (weighted-average cost, Opening Balance, Stock Out, Stock Movement evidence, price lists, dishes/extras, cash, payroll, reports) is tracked in #621, filed but not yet scheduled into a sprint. | PR #622 | `77855b8a` | 3h 16m | 133 PHPUnit + 11 Vitest passing; Pint/ESLint/TypeScript clean; Swagger regenerated; full CI (incl. 6 Cypress e2e shards) + both SonarCloud gates green; 2 Codex review rounds (2 P2 — scientific-notation float boundary, frontend/backend decimal-place mismatch; 1 P1 — PHP 8.3-only typed class constants breaking the composer.json-declared PHP 8.2 floor, caught only because CI tests PHP 8.5 exclusively — all fixed and threads resolved); merged to `main` (`77855b8a`). |
| ✅ | #579 | Added `Stock.total_value` — an exact running value accumulator maintained alongside the rounded, display-only `weighted_avg_cost` — so reversing a posted Purchase Receipt (`Stock::reverseWeightedAverageCost()`) now removes exactly the value it contributed, correctly across any number of sequential reversals, instead of leaving `weighted_avg_cost` untouched; refuses (409) rather than approximating when intervening consumption leaves unreconcilable residual value. Exposed valuation evidence on the #574 movement ledger detail endpoint. | PR #626 | `37e7bd7c` | 4.1h | 875+ PHPUnit tests passing (full suite 2,618/2,620 — 2 pre-existing unrelated `AttendancePayroll` test-order flakes, confirmed passing in isolation and failing identically without this branch's changes); Pint clean; 2 review rounds (1 Codex P1 — rounding-drift compounding across sequential reversals, fixed by introducing the `total_value` accumulator; 1 manual pass — migration overflow width, full-depletion negative-value clamp, forward-blend rounding-compounding, a generic-reverser valuation bypass, nullable-cost serialization — all fixed with direct regression tests); merged to `main` (`37e7bd7c`). |
| ✅ | #575 | Dropped the now-redundant `stock_movement_lines.item_variant_id`/`base_qty` — the header's own `item_variant_id`/`qty` is the single persisted source now. A reconciliation-then-drop migration aborts with the disagreeing line ids (exact SQL equality, no float tolerance) if any row disputes its header before dropping the FK/index/CHECK/columns; `down()` backfills losslessly from the header. Updated the four line writers, the model, tests, webapp types, and EN/ES architecture docs to match. | PR #619 | `2388df3b` | 38m | Full `Feature/Inventory` PHPUnit suite (788/788) and Vitest inventory-type suite passing; new `StockMovementLineColumnRemovalMigrationTest` (reconciliation abort incl. a one-representable-unit boundary case, happy-path drop, `down()`/`up()` round trip); Pint/ESLint/TypeScript clean; 2 Codex review threads resolved (a pre-existing preview-deploy migration gap correctly identified as out of scope, and a real float-tolerance precision bug fixed with a regression test); a rolling-deployment column-drop risk was raised and explicitly confirmed with the user (kept as a single-step drop, logged on the PR). |
| ✅ | #580 | Replaced the growing `permission:a\|b\|c` OR-strings on six reference-data lookup routes (Products, Variants, Variant Purchase Presentations, Inventory Locations, Suppliers, Supplier Offerings) with `App\Support\InventoryCatalogLookup` — one documented OR-list per domain, each including a new generic `inventory_catalog.lookup` permission a future workflow can be granted directly, without touching any route. Preserved the existing `suppliers.manage` (#505) and `receipts.manage` (#433) exceptions verbatim. Also extracted one shared frontend query hook for the Supplier Offering/Purchase Receipt Presentation-lookup cascade they duplicated identically. | PR #620 | `57f3b963` | 55m | Full `Feature/Inventory` PHPUnit suite (793/793) and full Vitest suite (4618/4618) passing unchanged; new `InventoryCatalogLookupPermissionTest` covers catalog viewer, supplier manager, receipt manager, the new generic-permission-only operator, unauthorized users, inactive Operating Unit membership, and admin bypass; Pint/ESLint/TypeScript clean. |
| ✅ | #581 | Audited every Inventory/Product/Purchasing/Pricing OpenAPI identifier against #399's ULID migration, correcting every stale integer-ID annotation; found and fixed 6 genuine runtime defects along the way (raw ID/FK leaks on Opening Balance, UOM Create/Conversions, Variant Purchase Presentations, and `GET /item-variants`, plus a malformed-input 500) | PR #623 | `e899db70` | 1h 3m | 753 Inventory + 59 Pricing PHPUnit tests passing; new `OpenApiIdentifierContractTest` (4 tests) gates future drift; Pint clean; 2 Codex review threads + 1 manually-found defect, all fixed and resolved |
| ✅ | #576 | Extended `DataGrid`/added `DetailStatus` as the shared loading/empty/error/permission/background-refetch state contract and wired all ten canonical Inventory screens (Productos, Insumos, Variantes, Ubicaciones, Existencias, Proveedores, Recepciones, Listas de precios, Transferencias, Movimientos) onto it, translated their chrome to Spanish, and fixed several real correctness bugs the wiring surfaced (Existencias fabricating zero totals/hiding cached data on a failed refetch, filters not resetting `currentPage`, `DataGrid` discarding still-usable cached rows on a transient error, and a `403`/`404` misclassification across 8 lists + the Existencias/Receipts/Transfers detail views that left revoked-access or deleted-record data visible). | PR #625 | `2b1d4f91` | 7h 44m | 356 Vitest files / 4711 tests passing; ESLint/TypeScript clean; full CI (webapp-ci, e2e-ci Cypress) green; 3 Codex review threads resolved across several rounds, each a real, verified defect (not a business-rule dispute) fixed with a regression test; merged to `main` (`2b1d4f91`). |
| ✅ | #577 | Split every canonical `/inventario/*` route's page component into a `*.lazy.tsx` sibling (`createLazyFileRoute`), keeping `beforeLoad`/`validateSearch`/loaders on the eager route file so permission guards always resolve before a chunk downloads; added one router-wide loading/failure boundary for lazy chunks with a full-reload retry path for a stale chunk after a deploy; documented the convention in `doc/conventions/frontend/routing-structure.md`. | PR #630 | `09f5fee0` | 18m | Production build: entry chunk 1,348.67 kB (359.93 kB gzip) → 1,103.38 kB (306.44 kB gzip), ~18% smaller, plus 10 new on-demand Inventory chunks; Vitest full suite 358 files / 4720 passing (2 pre-existing skips), 100% coverage on the two new files; ESLint/TypeScript clean; Cypress `inventory-navigation.cy.ts` (15/15) and `inventory-stock-movement-ledger.cy.ts` (1/1) green against the dev-lab E2E stack (`stock-transfers.cy.ts`'s 1 pending test is a pre-existing #548 quarantine, unrelated); merged to `main` (`09f5fee0`). |
| ✅ | #545 | Un-quarantined `item-media-gallery-uploader.cy.ts`; fixed the submit-button busy-state race, made the spec's SKU retry-safe, and fixed a shared CI-only `crypto.randomUUID` secure-context failure (also unblocks #552/#556) | PR #618 | `358fcc53` | 0.6h | 11/11 green on dev-lab E2E (6× default + 5× CI config, 0 retries); `e2e-ci / cypress-e2e-run` green; ESLint/TypeScript clean; 2 Codex findings addressed |
| ✅ | #546 | Removed the #490 `this.skip()` guard from `price-lists.cy.ts` and hardened its Cypress-retry safety — a per-attempt Price List purge plus a per-attempt `code` suffix (PriceList uses SoftDeletes, so the purge alone left the `code` unique index still claimed) — found and fixed across two Codex review rounds. | PR #616 | `32920599` | 1h 30m | 4/4 headless runs green against a fresh dev-lab E2E stack; no product code changed; the covered-`<select>` overlay flake no longer reproduces; 2/2 Codex review threads (retry-state purge, soft-delete/unique-code interaction) resolved; ESLint + `npm run typecheck` clean. |
| ✅ | #550 | Root-caused the `suppliers-catalog.cy.ts` failure to a backend defect, not test fragility — `GET /inventory/products/{id}/variants` rejected `is_active=true` with the bare `boolean` rule. Added `ListVariantsRequest` mirroring `ListProductsRequest`'s boolean-string tolerance, restored not-found-before-validation-error precedence on an unknown product id, then removed the #490 `this.skip()` guard. | PR #617 | `df0db57a` | 19m | 53/53 `ProductVariantCrudTest` assertions passing (3 new cases: boolean-string `is_active`, unsupported `is_active` value still 422, unknown product + invalid filter still 404); Pint clean; 1 Codex review thread (invalid-boolean silently nulled) + 1 manually-reported not-found-precedence finding, both fixed and resolved. |
| ✅ | #553 | Opportunistic (§5.4): restored `schedule-indefinite-summary.cy.ts` by scrolling directly to grouped-day and compact-range text, avoiding fixed-header scroll overshoot. Removed quarantine and the non-retriable navigation warm-up; merged to `main` (`720fead2`). | PR #629 | `720fead2` | 1.3h | Prior implementation evidence reports 3/3 cases passing, two CI runs green and two additional cold-cache local runs after removing the warm-up; 1/1 Codex thread resolved; finish-pr ESLint: 0 errors (31 warnings), TypeScript clean; final promoted CI checked on PR #629 |
| ✅ | #554 | Granted admin all Cash Adjustments permissions in all three seeders, restored cash-register code-suggestion E2E coverage, moved Devtools to the desktop top-left while hiding its launcher below lg to keep mobile navigation accessible, and made the spec's `test:reset` retry-safe against CI's `retries=2`. merged to `main` (`0388c24c`). | PR #631 | `0388c24c` | 0.3h | Prior implementation evidence: 123 CashAdjustments/seeder tests (321 assertions), AdminRoleCashAccessTest (2 tests, 31 assertions), 3 Chrome runs + 1 Electron run passing. Finish-pr: Pint passed (1340 files); ESLint 0 errors (32 warnings); TypeScript passed; 2/2 Codex review threads (Devtools/mobile-menu overlap, Cypress retry-state leak) addressed and resolved. Final promoted CI validation passed; merged to `main` (`0388c24c`). |
| ✅ | #535 | Root-caused the "employee name clipped by a parent" failure to a missing `scrollIntoView()` — the "Total" tab and broad default view render all 10 employees `AttendanceTestSeeder` creates (this spec's own seeder only layers attendance records on top of a subset), overflowing the viewport inside the scrollable `<main>` panel. Added `scrollIntoView()` calls matching the pattern already used by every other attendance spec, corrected the file's header comment, and removed the `#490` `this.skip()` quarantine guard. | PR #628 | `35bf5afd` | 18m | 10/10 Cypress tests passing, run twice in a row against a fresh `make e2e WORKSPACE=sushigo-b` stack to confirm reliability; ESLint/TypeScript clean; 3 Codex review threads resolved (a stale sprint-evidence row, a Total-tab test only asserting 5 of 10 employees, and the §5.1/§13 status-table mismatch that followed from the first); rebased once past 3 concurrently-merged quarantined-spec PRs (`#553`, `#554`, `#577`) touching the same sprint doc; merged to `main` (`35bf5afd`). |
| ✅ | #552 | Opportunistic (§5.4): removed the `#490` quarantine guard from `employee-avatar-upload.cy.ts` — the actual root cause (Chrome's `crypto.randomUUID()` secure-context failure on `e2e-ci`'s non-localhost origin) had already been fixed by #545's shared `cypress.config.ts` exemption, so no app/config change was needed here. Also fixed an independent submit-button busy-state race (same pattern as #545) and, after Codex review, a retry-safety issue (fixed employee email/name colliding across a `retries=2` CI rerun). | PR #627 | `1116db34` | 18m | 3/3 green on a fresh dev-lab E2E stack; no regression in `item-media-gallery-uploader.cy.ts`/`employees.cy.ts`; ESLint/TypeScript clean; 1/1 Codex review thread resolved (retry-safety finding); rebased twice past concurrently-merged sibling quarantine-spec PRs (`#553`, `#554`, `#535`) touching the same sprint doc; merged to `main` (`1116db34`). |
