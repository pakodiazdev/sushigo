...
**Progress as of 2026-08-08:** 12 of 14 scoped Issues completed (85.7%) — `#384` (Critical
...
component, PR #407) merged to `main`; `#357` (Attendance Today card exit animation, PR #397)
implemented and fully reviewed, PR open and ready for merge.
...
| Calendar duration | — |
| Active workdays | — |
| Progress (Issues completed) | 12 / 14 (85.7%) as of 2026-08-08 — `#384` (PR #393), `#385` (PR #391), `#358` (PR #395), `#376` (PR #396), `#383` (PR #398), `#379` (PR #394), `#377` (PR #392), `#382` (PR #408), `#381` (PR #406), and `#378` (PR #407) merged to `main`; `#357` (card exit animation, PR #397) implemented and fully reviewed, PR open and ready for merge |
...
| ✅ | #379 | Build the Platillos (dishes) backend domain: categories, dishes, extras | High | 5h | 10h | 8.9h | PR #394 | Full CRUD for categories/dishes/extra groups/extra options, cascading soft-deletes wrapped in transactions, ULID public_id, per-entity subfolders, 20 endpoints + Swagger; merged to `main` (3aff508) |
| ✅ | #358 | Employees on vacation or a scheduled rest day today don't appear under "Ausentes" | High | 3h | 6h | 5.2h | PR #395 | `today_vacation` backend field + `isAbsentRow`/`isHiddenFromGrid` frontend fallback; merged to `main` (c79a46c) |
...
| ✅ | #357 | Unify card exit/transition animation across all Attendance Today state changes | Medium | 2h | 4h | 33.0h | PR #397 | Generalized `cardOverrides`/`startExitAnimation` into a shared bucket-parameterized trigger for check-in/lunch-start/lunch-return/check-out; Devin review found and fixed 3 real bugs (stale-tab closure, flash-back-after-fade, hidden-card-stuck) with regression tests; follow-up polish made the exit animation more pronounced and added a FLIP-based grid reflow so sibling cards slide instead of snapping — PR ready, merge pending |
...
| ✅ | #357 | Generalized the `cardOverrides`/`startExitAnimation` mechanism into a bucket-parameterized `staysInActiveTab` trigger so check-in, lunch-start, lunch-return, and check-out all play the same fade/slide-out exit animation as the existing mark-falta flow; follow-up session made the exit animation more pronounced and added a FLIP-based `useAttendanceGridFlip` hook so sibling cards reflow fluidly instead of snapping — PR open, not yet merged | PR #397 | — | 33.0h | 111 Vitest passing (17 new: fresh-action animation, correction no-animation, stale-tab safety, flash-back prevention, hidden-card reveal, full-day-leave target bucket), full 3627-test suite green, lint + typecheck clean; Copilot review fixed 1 real defect (test timer pattern); Devin/DeepWiki review loop (4 cycles) found and fixed 3 real bugs beyond original scope (stale-tab-switch closure, flash-back-after-fade, hidden-card-permanently-stuck), each with a dedicated regression test — final confirmation scan didn't complete within Devin's free-tier time budget, so this was resolved via the pipeline's own test verification instead; CI 12/12 green; follow-up polish re-verified with 198/198 Vitest passing (attendance page + card component suites), lint/typecheck clean, and manual browser check-in testing; `/finish-pr`'s Devin re-scans then found and fixed 13 more real bugs across eight rounds (fast-refetch hidden-card race, FLIP wrapper breaking equal card height, stale FLIP positions after window resize, FLIP scroll-drift (window.scrollY, later found to be a no-op and properly fixed with container-relative measurement), mark-falta hidden-card race, mark-falta silent-failure card loss, StrictMode updater-purity stuck-hidden bug, date/branch-switch state leak, mark-falta double-submit window, FLIP content-height staleness, ResizeObserver initial-notification mid-transform race, FLIP scroll compensation targeting the wrong scroll container, dead confirm-dialog isLoading + missing check-in guard during mark-falta) — round 3 replaced the reference-comparison hide/reveal mechanism entirely with a semantic attendanceBucket() check to close that whole bug class at its root; final rebase onto `main` after #382's merge (PR #408) resolved doc-only progress-counter conflicts as set unions; a ninth Devin pass then found the eighth pass's own fix disabled every card's buttons page-wide instead of just the acted-on employee's, fixed by tracking the specific employee id being marked; two further rebases (issues #382/#381, then #378) resolved the same sprint-doc conflict pattern; a tenth pass then found that single-scalar-id fix itself broke under overlapping absence writes for two different employees, corrected by tracking a `Set<string>` of in-flight ids instead; an eleventh pass found the `'hidden'` override had no fallback if its guessed target bucket never materialized (e.g. a concurrent leave approval), fixed with a 35s backstop timer; a twelfth pass found the same `'hidden'` state also blocked the "Ver todos" empty-grid escape hatch by suppressing rows even under the "Total"/default view, fixed by only suppressing on bucket-specific tabs; a thirteenth pass found two more real bugs: in-flight mutations weren't guarded against a date/branch switch (fixed with selectedDateRef/branchIdRef mirrors, same pattern as the existing selectedFilterRef) and the FLIP ResizeObserver could re-measure a card mid-transition (fixed by tracking animating employee ids and skipping their re-measurement); a fourteenth pass found the mandatory CLAUDE.md Cypress happy-path spec was missing (added cypress/e2e/attendance-exit-animation.cy.ts, verified passing against the real dev-lab E2E stack) and that the FLIP layout effect's own baseline measurement had the same mid-transition contamination as the ResizeObserver (fixed with measureLayoutPositions, neutralizing transforms pre-paint), full 3749-test suite green — PR ready, merge pending |
| ⏳ | #365 | Not started | — | — | — | — |
