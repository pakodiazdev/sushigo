...
**Progress as of 2026-08-11:** 11 of 14 scoped Issues completed (78.6%) — `#384` (Critical
...
component, PR #407) merged to `main`; `#365` (local pre-PR testing convention — linters +
delivered tests only, full suite is CI's job, PR #413) has its PR ready, merge pending.
...
| Calendar duration | — |
| Active workdays | — |
| Progress (Issues completed) | 11 / 14 (78.6%) as of 2026-08-11 — `#384` (PR #393), `#385` (PR #391), `#358` (PR #395), `#376` (PR #396), `#383` (PR #398), `#379` (PR #394), `#377` (PR #392), `#382` (PR #408), `#381` (PR #406), and `#378` (PR #407) merged to `main`; `#365` (PR #413) PR ready, merge pending |
...
| ⏳ | #357 | Not started | — | — | — | — |
| ✅ | #365 | Reworded `doc/conventions/git/pr_review_rules.md`, `doc/conventions/testing/testing-strategy.md`, and `doc/TESTING.md` so the local pre-PR step is "linters + delivered tests" with the full suite reserved for CI, and added the objective-fix rule for CI-detected regressions | PR #413 | — | 0.02h | Docs-only change, no tests/linters applicable; Copilot review fixed 6 issues in one follow-up commit (incorrect "workspaces share `mydb_test`" claim contradicted by `code/api/phpunit.xml`/`.env.testing`'s per-workspace isolation, missing Docker `cd`/`DB_DATABASE` overrides, `npx vitest run` consistency), all replied to and resolved — PR ready, merge pending |
