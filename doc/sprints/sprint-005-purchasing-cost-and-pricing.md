...
| Status | Issue | Title | Value | Opt. | Pess. | Tracked | PR / Commit | Notes |
|---|---:|---|---|---:|---:|---:|---|---|
| ✅ | #433 | Add purchase receiving UI and cost preview | High | 6h | 11h | 2.92h | PR #515 | Receipt list/create-edit-draft/immutable posted-reversed detail UI shipped, preview matches #432's formula exactly, 5 review threads + 3 manually-found bugs resolved, 2 SonarCloud smells cleaned up — PR ready, merge pending |
| ✅ | #434 | Unify weighted-average acquisition cost | Critical | 4h | 8h | 1.3h | PR #514 | Stock.weighted_avg_cost (per Inventory Location) is now the single source, exact-decimal via bcmath, legacy Variant fields frozen and reconciled — PR ready, merge pending |
|  |  | **Round total** |  | **10h** | **19h** | **—** |  |  |
