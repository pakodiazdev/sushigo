...
- [#378](https://github.com/pakodiazdev/sushigo/issues/378) — la contraparte frontend reutilizable de este sistema: `<MediaGalleryUploader />` + `useMediaGalleryUploader()` (`code/webapp/src/components/media/`), integrada de extremo a extremo en `ItemForm` (el participante "Client (Webapp)" de §7.1). Cualquier futuro adoptante (Platillos, avatares de Employee/User) reutiliza este componente en vez de reimplementar la lógica de subida y tracking de la galería.
