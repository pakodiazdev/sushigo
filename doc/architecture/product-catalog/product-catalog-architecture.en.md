...
|---|---|---|---|---|
| GET | `/inventory/products` | — (query: `search, brand_id, inventory_category_id, is_active, page`) | `items.view` \| `suppliers.manage` | Filters to `type = PRODUCTO` server-side; returns variant count per row for the list. **As-built (`#505`):** also accepts `suppliers.manage` — the Suppliers offering-create cascade needs this list for a user authorized to manage supplier offerings but not the general catalog. |
...
| GET | `/inventory/products/{product}/variants` | — (query: `per_page`) | `items.view` \| `suppliers.manage` | **As-built (`#424`):** no `search`/`is_active` filters — always scoped to the parent Product's own variants. **As-built (`#505`):** also accepts `suppliers.manage`, same reasoning as the Products list row above. |
...
| GET | `/inventory/products/{product}/variants/{variant}/purchase-presentations` | — | `items.view` \| `suppliers.manage` | **As-built (`#505`):** also accepts `suppliers.manage`, same reasoning as the Products list row above. |
