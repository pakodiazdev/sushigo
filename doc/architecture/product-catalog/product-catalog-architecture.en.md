...
|---|---|---|---|---|
| GET | `/inventory/products` | — (query: `search, brand_id, inventory_category_id, is_active, page`) | `items.view` \| `suppliers.manage` \| `receipts.manage` | Filters to `type = PRODUCTO` server-side; returns variant count per row for the list. **As-built (`#505`):** also accepts `suppliers.manage` — the Suppliers offering-create cascade needs this list for a user authorized to manage supplier offerings but not the general catalog. **As-built (`#433`):** also accepts `receipts.manage`, same reasoning — the Purchase Receipt form's own Product cascade. |
...
| GET | `/inventory/purchase-presentation-templates/suggest-code` | Query: `package_type, base_unit_quantity, compatible_dimension_uom_id` | `purchase_presentation_templates.manage` | Create-mode convenience; returns one available semantic code and never mutates a persisted template. |
...
| GET | `/inventory/products/{product}/variants/{variant}/purchase-presentations` | — | `items.view` \| `suppliers.manage` \| `receipts.manage` | **As-built (`#505`):** also accepts `suppliers.manage`, same reasoning as the Products list row above. **As-built (`#433`):** also accepts `receipts.manage`, same reasoning. |
...
**As-built (`#499`) — semantic template codes:** the composition is
`PACKAGE_TYPE` + `_` + quantity normalized to the column's four-decimal scale with trailing zeroes
removed (`24`, `24.0`, and `24.0000` all become `24`; `2.5000` becomes `2.5`). On collision the
compatible UOM code is appended (`BOX_24_KG`), followed by a numeric suffix starting at `_2`.
Tokens are ASCII uppercase with non-alphanumeric runs changed to `_`; candidates are truncated as
needed to remain within 50 characters. Suggestions treat soft-deleted codes as occupied, preserving
historical meaning. The create request still accepts manual codes. Its partial unique index is the
final authority; a concurrent collision returns the standard 422 field error plus `rejected_code`
and a fresh `suggested_code`, and requires explicit resubmission.
