...
- The **same transaction** also ensures the `VariantLocationAssignment` for the pair (§3.13, #569) —
  creating it, or reactivating a soft-deleted one — via the shared `VariantLocationAssignmentEnsurer`
  service (§3.13, #572). Unlike the `PUT …/variant-assignments/{variantId}` endpoint, initialization does **not**
...
| Availability preview | `POST /inventory/transfers/preview` (#613) — non-authoritative: a single plain `Stock` read (no lock, no write) for the source Location + Variant, plus the same entry→base conversion `createLine()` uses. The line editor shows it as an indicative on-hand/available figure and normalized base-UOM quantity; the authoritative check still happens under lock at post time, so a concurrent change between preview and post is expected, not a defect. |
...
>
> **As-built note (`#399`/`#495`, superseding the above):** `#399` has landed — `Item`,
> `ItemVariant`, `InventoryLocation`, `UnitOfMeasure`, `Stock`, `StockMovement`,
> `StockMovementLine`, `StockTransfer`, `StockTransferLine` and the rest of this domain now use
> `public_id` (ULID) the same way `Dish`/`MediaGallery`/`CashAdjustment` do. See
> [#581](https://github.com/pakodiazdev/sushigo/issues/581) for the follow-up audit that corrected
> the OpenAPI documentation drift left behind by that migration.
