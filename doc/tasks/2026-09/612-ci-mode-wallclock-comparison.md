...
open investigation needed. The real time saver was not having to *generate* fresh CI data:
`gh run view --json jobs` against my own two most recent PRs (#658, #660) — each of which already
had both a draft `[ci-check]` run and a promoted `[ci-check-all]` run on record — supplied every
figure the artifact needed directly from real timestamps, with zero new CI spend. The only
non-trivial work was arithmetic care (three separate rounds of re-verifying wall-clock deltas with
`python3 -c` after catching my own eyeballing errors) and cross-checking the shard-imbalance
finding against `_e2e-ci.yml`'s code comments and `testing-strategy.md`'s existing #559 analysis
before writing it up, so the new section reconfirms rather than duplicates or contradicts prior
work.
